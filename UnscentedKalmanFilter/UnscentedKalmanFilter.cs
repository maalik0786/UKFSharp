...
public class UKF
{
	...
	public void Update(double[] measurements)
	{
		...
		var P12 = X2.Multiply(Matrix.Build.Diagonal(Wc.Row(0).ToArray())).
			Multiply(Z2.Transpose());
	...
	public double[] getState() => x.ToColumnArrays()[0];
	public double[,] getCovariance() => P.ToArray();
	...
	private Matrix<double>[] UnscentedTransform(Matrix<double> X, Matrix<double> Wm,
		Matrix<double> Wc, int n, Matrix<double> R)
	{
		var L = X.ColumnCount;
